...
def find_shortest_route(connections: str) -> list[list[int]]:
    ...
    # Perform BFS to find the shortest route
    routes = []
    visited = set()
    queue = [(1, [1])]  # Start from city 1
    while queue:
        city, path = queue.pop(0)
        if city == len(graph):  # End at the highest indexed city
            routes.append(path)
        else:
...
shortesPaths = find_shortest_route(connections)  # should print [1, 2, 4, 6, 100]
