def assign_rooms(groups):
  ...
  for group in groups:
    ...
    room_found = False
    for room in rooms:
        if room['size'] + group <= 10:
            # Assign the group to this room
            room['size'] = room['size'] + group
    ...
    if not room_found:
      ...
      rooms.append({'size': group, 'groups': [group]})
...
if __name__ == "__main__":
    ...
    room_placement = assign_rooms(splitInputData)

    for line in room_placement:
      print(str(line['groups']).replace("[", "").replace("]", "").replace(",", ""))
